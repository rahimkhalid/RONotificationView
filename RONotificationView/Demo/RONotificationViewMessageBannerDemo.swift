//
//  RONotificationViewMessageBannerDemo.swift
//  RONotificationView
//
...
class RONotificationViewMessageBannerDemo: UIViewController {
    ...
    @IBOutlet weak var showLoaderSwitch: UISwitch!
    
    var windowBanner: RONotificationView!
    var controllerBanner: RONotificationView!
    var viewBanner: RONotificationView!
    var configuration: RONotificationMessageConfiguration!
    
    override func viewDidLoad() {
        configuration = RONotificationMessageConfiguration(title: titleTxtField.text ?? "", message: messageTxtField.text ?? "", isToAnimateView: animateSwitch.isOn)
    }
    
    func createConfiguration(){
        configuration.title = titleTxtField.text ?? ""
        configuration.isToShowLoader = showLoaderSwitch.isOn
        ...
        configuration.titleTextColor = colors[Int(textColorSlider.value)]
    ...
    @IBAction func showBannerOnWindow(_ sender: UIButton) {
        if windowBanner == nil{
            windowBanner = RONotificationMessageBanner(configuration)
        }
        ...
        if windowBanner.isVisiable {
            ...
            createConfiguration()
            configuration.image = UIImage(named: "danger2x")!
    ...
    @IBAction func showBannerOnView(_ sender: UIButton) {
        if viewBanner == nil{
            viewBanner = RONotificationMessageBanner(presentOn: configurationView, config: configuration)
        }
    ...
    @IBAction func showBannerOnController(_ sender: UIButton) {
        if controllerBanner == nil{
            controllerBanner = RONotificationMessageBanner(presentOn: self, config: configuration)
        }
